            ...
            for (var i =0; i < object_process.length; i++){
                if(i > 0){
                    ...
                    for(var j =0; j < set_condition.length; j++){
                        if(j == 0){
                            var rule_applied_field = set_condition[j].replace("[", "").replace("]", "");
                        ...
                        var and_condition_string = set_condition[j];
                        var if_field_input_value = and_condition_string.match(/\[(.*?)\]/g);
                        var operator_input_value = and_condition_string.match(/equal|not-equal/g);
                        var if_type_input_value  = /if-\[/g.test(and_condition_string) ? "" : and_condition_string.match(/and|or/g);
                        var if_value_input       = and_condition_string.match(/".*?"/g);
                        console.log(and_condition_string);
                        condition_object.and_condition_rules.push({
                            if_field_input : if_field_input_value[0].replace("[", "").replace("]", ""),
                            if_type_input  : Array.isArray(if_type_input_value) ? if_type_input_value[0] : if_type_input_value,
                            operator_input : operator_input_value[0],
                            if_value_input : if_value_input[0].replace('"', '').replace('"', ''),
                        });
                    }
                    condition_as_object.push(condition_object);
                }
            }
            console.log(condition_as_object);
